...
/* Menu links may carry a hash (/markets#forex); the page is what counts. */
const pathOf = (href) => (href ? href.split('#')[0] : null);
    ...
    >
      <div className="shell">
        <div className="masthead__inner">
          <SmartLink href="/" className="masthead__mark" aria-label={`${site.name} home`}>
            ...
            {navigation.map((entry) =>
              entry.groups ? (
                          ...
                          <ul>
                            {group.items.map((item) => (
                              <li key={item.label}>
                                <SmartLink
                                  href={item.href}
                                  className={item.icon ? 'mega__link mega__link--icon' : 'mega__link'}
                                ...
                                >
                                  {item.icon ? (
                                    <>
                                      <span className="mega__icon" aria-hidden="true"><Icon name={item.icon} size={18} /></span>
                                      <span className="mega__text"><b>{item.label}</b>{item.note ? <small>{item.note}</small> : null}</span>
                                    </>
                                  ) : (
                                    <>
                                      <b>{item.label}</b>
                ...
                <div key={entry.id} className="nav__item">
                  <SmartLink
                    href={entry.href}
                    className={entry.highlight ? 'nav__trigger nav__trigger--highlight' : 'nav__trigger'}
          ...
          <div className="tools">
            ...
            <SmartLink href={site.loginUrl} className="btn btn--ghost btn--sm masthead__login" onClick={() => { setDrawer(false); setOpenId(null); }}>
              Login
            </SmartLink>

            <SmartLink href={site.liveAccountUrl || "/trading/account-types"} className="btn btn--solid btn--sm masthead__cta">
              Open live account
            </SmartLink>
