...
export function ThemeArt({ src, light, frameRatio, alt = '', priority = false, className = '', sizes = '(max-width: 760px) 90vw, 50vw' }) {
  const lightSource = light || (src === '/assets/partner/bytefx-glass-mark.webp' ? '/assets/generated/bytefx-glass-mark-light.png' : undefined);
  return <div className={`theme-art ${className}`} data-paired={Boolean(lightSource)}><Image className="theme-art__dark" style={frameRatio ? { aspectRatio: frameRatio } : undefined} unoptimized={src.startsWith('/assets/supplied/')} src={src} alt={alt} width={1100} height={1100} sizes={sizes} priority={priority} />{lightSource && <Image className="theme-art__light" unoptimized={lightSource.startsWith('/assets/supplied/')} src={lightSource} alt={alt} width={1100} height={1100} sizes={sizes} />}</div>;
}
