          ...
          <div className="atlas-conversation" ref={conversation} role="log" aria-label="Conversation with Atlas" aria-live="polite" aria-relevant="additions">
            {messages.map((message,index)=>(
              <div key={index} className={`atlas-message atlas-message--${message.role}`}>
                ...
                <p>{message.text}</p>
                {message.links && <div className="atlas-message__links">{message.links.map(link=><a key={link.label} href={link.href} {...(link.href.startsWith('https://')?{target:'_blank',rel:'noopener noreferrer'}:{})} onClick={()=>{if(link.href.startsWith('#') || link.href.startsWith('/'))close();}}>{link.label}<Icon name="arrow" size={13} /></a>)}</div>}
              </div>
            ))}
          </div>
