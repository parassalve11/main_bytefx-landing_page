...
export default function ClosingSection({ title, accent, lead, primary = cta.primary, secondary = cta.secondary }) {
    ...
    <section ref={section} className="closing closing--markets" id="get-started" aria-labelledby="closing-title">
      <div className="closing__scene" aria-hidden="true">
        <div className="closing__wing closing__wing--left">
          <Image className="theme-art--dark" src={cta.background} alt="" fill sizes="100vw" />
          <Image className="theme-art--light" src="/assets/generated/blue/closing-market-scene.png" alt="" fill sizes="100vw" />
        </div>
      ...
      <Reveal className="closing__inner">
        ...
        <h2 className="h-xl" id="closing-title">
          {title || 'Ready to start'}<br /><span className="tint">{accent || 'trading?'}</span>
        </h2>
        <p className="closing__lead">{lead || cta.lead}</p>
        <div className="closing__acts">
          <SmartLink href={primary.href} className="btn btn--solid">
            {primary.label}<Icon name="arrow" size={16} />
          </SmartLink>
          <SmartLink href={secondary.href} className="btn btn--ghost">{secondary.label}</SmartLink>
        </div>
